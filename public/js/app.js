...
async function loadCategories() {
  ...
  $("#category-filters").innerHTML = state.categories.filter((c) => c.active).map((c) => `<button class="filter" data-category="${c.code}"><span><i class="category-dot" style="background:${c.color || "#999"}"></i>${escapeHtml(c.name)}</span></button>`).join("");
...
async function loadConversations() {
  ...
  $("#count-all").textContent = state.conversations.length;
  $("#count-new").textContent = state.conversations.filter((c) => c.status === "NOVO").length;
